...
- **หน้าตัดคาน scale ตามสัดส่วน b×h จริง + วงกลมเหล็กสะท้อนขนาดเบอร์** (audit ความถูกต้องเชิงเรขาคณิต) — เดิม fix viewBox 130×150 → คานสูง/แบนวาดรูปเดียวกัน · เดิมทุกเบอร์รัศมีเท่ากัน (DB10 = DB28). แก้: กล่องคอนกรีต scale `W=b·k, H=h·k` (fit-box คงสัดส่วน) · ปลอก/เหล็ก inset = (cover+ปลอก) scale จริง · รัศมีวงกลม ∝ db (เบอร์ใหญ่=วงใหญ่ · เบอร์ผสมวาดตามจริง ใหญ่ไปมุม) · ใช้ logic จาก reference `rc-beam-section.js` + vault (verified vs DRMK ตาราง 3.3) · n_layers ยังมาจาก engine · index-only ไม่แตะ engine
### ภายใน
- `runDesignAudit` 50→52→**56** (guard เบอร์เหล็ก + guard แยกหลัก/เสริม + guard สัดส่วนหน้าตัด ≈ b/h จริง (G1) + guard เบอร์ผสม→วงหลายขนาด (G2) · เปลี่ยน guard นับเหล็กหน้าตัดเป็นนับตาม `data-bar` attribute robust ต่อ geometry scale) · index-only
